...
const CardItem: React.FC<{ card: TarotCard, size?: 'sm' | 'md' | 'lg', isCrossing?: boolean }> = ({ card, size = 'md', isCrossing = false }) => (
  <div
      ...
      <section className="space-y-6">
        ...
        {isCelticCross ? (
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8">
            ...
            <div className="rotate-90 md:rotate-0">Right</div>
          ...
          <div className={`grid gap-6 ${cards.length === 1 ? 'grid-cols-1 max-w-xs mx-auto' : 'grid-cols-1 sm:grid-cols-3'}`}>
            {cards.map((c, i) => {
              const cardKey = `${c.name}-${c.isReversed}-${i}`;
              return <CardItem key={cardKey} card={c} size="md" isCrossing={false} />;
            })}
          </div>
        )}
      </section>
