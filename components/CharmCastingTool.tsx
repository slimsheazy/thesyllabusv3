...
const CHARMS = [
  { id: 'sun', icon: 'Sun', name: 'The Sun' },
  { id: 'moon', icon: 'Moon', name: 'The Moon' },
  { id: 'key', icon: 'Key', name: 'The Key' },
  { id: 'heart', icon: 'Heart', name: 'The Heart' },
  { id: 'skull', icon: 'End', name: 'The End' },
  { id: 'star', icon: 'Star', name: 'The Star' },
  { id: 'eye', icon: 'Eye', name: 'The Eye' }
];
      ...
      if (!charm.settled) {
        ...
        if (charm.scale > 1) {
          ...
          if (charm.scale <= 1) {
            charm.scale = 1;
            if (!charm.hasClanked) {
              playClank(0.12); charm.hasClanked = true;
            }
          }
        }
        ...
        if (distFromCenter + radius > boardRadius) {
          ...
          if (Math.hypot(charm.vx, charm.vy) > 0.5) {
            playClank(0.03);
          }
        }
  ...
  const handleInterpretation = async() => {
    ...
    const charmDataList: CharmData[] = charmsRef.current.map(c => {
      ...
      if (dist < 60) {
        ...
        ring = 'Internal Narrative';
      } else if (dist > 250) {
        ring = 'Universal Collective';
      }
