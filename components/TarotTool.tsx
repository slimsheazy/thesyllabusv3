...
const SPREAD_CONFIGS: Record<SpreadType, string[]> = {
  ...
  [SpreadType.CELTIC_CROSS]: [
    'Current state', 'Obstacles', 'Objectives', 'Foundation',
    'Recent past', 'Near future', 'Internal state', 'External perception', 'Aspirations & Concerns', 'Resolution'
  ]
};
...
export const TarotTool = ({ onBack }: { onBack: () => void }) => {
  const [question, setQuestion] = useState('General life guidance and insight');
  ...
  const handleDraw = async() => {
    ...
    const systemInstruction = `You are a tarot interpreter. Deck: ${deckLabel} (publicly available). Spread: ${spreadType}. Provide analytical interpretation of card connections. Use objective language. Conclude with 'ANALYSIS:' and a concise summary.${resonancePrompt}`;
      ...
      <div className="flex flex-col lg:flex-row gap-12 xl:gap-24 items-start pt-12 md:pt-0">
        <div className="w-full lg:w-[450px] space-y-10 lg:sticky lg:top-20">
          <header className="space-y-4">
            <h2 className="heading-marker text-6xl md:text-7xl text-marker-orange lowercase leading-none">Read the Cards</h2>
            <p className="handwritten text-xl opacity-40 uppercase tracking-widest italic">Finding the story in the deck</p>
          </header>
          <div className="space-y-6">
            <textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="Enter query for analysis..." className="w-full p-6 text-xl min-h-[140px] shadow-sm italic outline-none bg-surface/50 rounded-lg" />
