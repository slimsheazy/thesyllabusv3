...
const SynchronicityDecoder: React.FC<ToolProps> = ({ onBack }) => {
  ...
  const [formData, setFormData] = useState({
    description: 'Synchronicity observed in daily life',
    category: 'Numerical Pattern',
    emotionalState: 'Heightened awareness',
    significance: 3
  });
  ...
  const loadHistory = async() => {
    ...
    if (logs) {
      setSynchronicities(logs.map(l => JSON.parse(l.result)));
    }
  };
        ...
        <aside className="w-full lg:w-[400px] space-y-12 lg:sticky lg:top-12">
          ...
          <div className="p-8 marker-border border-dashed border-marker-black/10 bg-black/[0.01] space-y-4">
            ...
            <p className="handwritten text-sm italic text-marker-black/40 leading-relaxed">
               This tool maps object traces against planetary hours and numerical roots to expose logical frameworks in apparent randomness.
            </p>
          </div>
        </aside>
