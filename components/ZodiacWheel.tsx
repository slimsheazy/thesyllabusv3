  ...
  const layers = useMemo(() => {
    ...
    for (let i = 0; i < planets.length; i++) {
      for (let j = i + 1; j < planets.length; j++) {
        ...
        let color = "";
        if (diff <= 8) color = "var(--marker-purple)"; // Conjunction
        else if (Math.abs(diff - 120) <= 8) color = "var(--marker-green)"; // Trine
        else if (Math.abs(diff - 60) <= 6) color = "var(--marker-blue)"; // Sextile
        else if (Math.abs(diff - 90) <= 8 || Math.abs(diff - 180) <= 8) color = "var(--marker-red)"; // Square/Opp
        
        if (color) aspects.push({ p1: polar(80, d1), p2: polar(80, d2), color });
      }
    }
