...
import { PlanetaryHour, PlanetaryHoursResult } from '../types';

const PLANET_SYMBOLS: Record<string, string> = {
  Sun: '☉', Moon: '☽', Mars: '♂', Mercury: '☿', Jupiter: '♃', Venus: '♀', Saturn: '♄'
};

interface PlanetaryAIAnalysis {
  governance: string;
...
const PlanetaryHoursTool: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const [data, setData] = useState<PlanetaryHoursResult | null>(null);
  const [analysis, setAnalysis] = useState<PlanetaryAIAnalysis | null>(null);
  const [loading, setLoading] = useState(false);

  const calculate = useCallback(async() => {
    if (!isCalibrated || !userLocation) {
      return;
    }
  ...
  return (
    <ToolShell
      ...
      secondaryAction={{
        label: 'Recalculate',
        onClick: () => {
          audioManager.playRustle(); calculate();
        }
      }}
    >
      {data && (
