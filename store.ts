  ...
  persist(
    (set, get) => ({
      ...
      setUserLocation: (loc) => {
        ...
        if (userIdentity && userBirthday && loc) set({ isCalibrated: true });
      },
      setUserIdentity: (id) => {
        set({ userIdentity: id });
        const { userBirthday, userLocation } = get();
        if (id && userBirthday && userLocation) set({ isCalibrated: true });
      },
      setUserBirthday: (dob) => {
        set({ userBirthday: dob });
        const { userIdentity, userLocation } = get();
        if (userIdentity && dob && userLocation) set({ isCalibrated: true });
      },
